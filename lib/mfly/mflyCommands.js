/**
 * mflyCommands v1.3.1 | (c) 2013-2015, Mediafly, Inc.
...
var mflyCommands = function() {
    ...
    return {
        ...
        // added this to use instead of promise
        getFolderv1: function (_id, _callback) {
            doGetRequest(prefix + "data/folder/" + _id, _callback);
        },

        getItemv1: function (_id, _callback) {
            doGetRequest(prefix + "data/item/" + _id, _callback);
        },

        getItem: function (_id, _callback) {
            doGetRequest(prefix + "data/item/" + _id, _callback);
        },
        ...
        embed: function ($e, id, page) {
            if (mflyCommands.isWindows8()) {
                _internalEmbedWindows8(id, function (ajaxHandler, statusCode, response) {
                        if (statusCode === 301) {
                            $e.attr('src', response.getResponseHeader('x-Location'));
                        }
                    },
                    function (ajaxHandler, data, response) {
                        // ERROR HANDLER
                    });
            } else {
        ...
        getData: function (id) {
            if (mflyCommands.isWindows8()) {
                return $.Deferred(function (dfd) {
                    _internalEmbedWindows8(id, function (ajaxHandler, statusCode, response) {
                            if (statusCode === 301) {
                                var url = response.getResponseHeader('x-Location');
                                $.ajax({
                                    url: url,
                                    contentType: "text/plain charset=utf-8",
                                    success: function (data, textStatus, request) {
